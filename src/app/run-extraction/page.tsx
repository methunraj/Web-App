  ...
  for (const key in obj) {
    if (obj.hasOwnProperty(key)) {
      ...
      if (obj[key] === null || obj[key] === undefined) {
        ...
        // Handle arrays - convert to string or flatten if objects
        if (obj[key].length === 0) {
          flattened[newKey] = '';
        } else if (typeof obj[key][0] === 'object') {
          // Array of objects - create separate columns for each array item
    ...
    if (result.extractedData && result.status === 'success') {
      try {
        ...
        if (parsedData && typeof parsedData === 'object') {
          // Flatten the nested structure for Excel columns
          const flattenedData = flattenObject(parsedData);
          allDataObjects.push({ ...baseData, ...flattenedData });
        } else {
        ...
        allDataObjects.push({ 
          ...baseData, 
          extractedData_error: 'Invalid JSON format', 
          rawExtractedData: result.extractedData?.substring(0, 1000) // Limit length
        });
      }
    } else {
  ...
  for (let C = range.s.c; C <= range.e.c; ++C) {
    ...
    for (let R = range.s.r; R <= range.e.r; ++R) {
      ...
      if (cell && cell.v) {
        ...
        maxWidth = Math.max(maxWidth, Math.min(cellLength, 50)); // Cap at 50 chars
      }
    }
